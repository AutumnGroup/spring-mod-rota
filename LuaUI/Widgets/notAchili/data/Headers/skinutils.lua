...
function DrawButton(obj)
  ...
  local color = obj.selected and obj.selectedColor or obj.normalColor
  
  if obj.state=="pressed" then
    gl.Color( mulColor( color, 0.4 ) )
  elseif obj.hovered then
	gl.Color( mulColor( color, 0.7 ) )
  else
    gl.Color( color )
  end
...
function DrawTrackbar(self)
  ...
  TextureHandler.LoadTexture(0,self.ThumbImage,self)
    ...
    local my = y+h*0.5
    gl.TexRect( math.ceil(mx-tw*0.5), math.ceil(my-th*0.5),
				math.ceil(mx+tw*0.5), math.ceil(my+th*0.5), false, true )
