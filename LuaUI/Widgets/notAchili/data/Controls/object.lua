    ...
    if (i ~= "inherited") then
      ...
      if (t=="table")or(t=="metatable") then
        ...
        if (ot ~= "table")and(ot ~= "metatable") then
          Spring.Echo("NotAchili: " .. obj.name .. ": Wrong param type given to " .. i .. ": got " .. ot .. " expected table.")
          obj[i] = {}
        end
...
function Object:AddChild(obj, dontUpdate)
  ...
  if (self.children[objDirect]) then
    --if (self.debug) then
      Spring.Echo("NotAchili: tried to add the same child multiple times (".. (obj.name or "") ..")")
  ...
  if (obj.name) then
    if (self.childrenByName[obj.name]) then
      error(("NotAchili: There is already a control with the name `%s` in `%s`!"):format(obj.name, self.name))
      return
    end
