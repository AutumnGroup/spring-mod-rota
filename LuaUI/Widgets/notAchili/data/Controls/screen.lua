...
function Screen:IsAbove(x,y,...)
  ...
  if (UnlinkSafe(hoveredControl) ~= UnlinkSafe(self._lastHoveredControl)) then
	...
	local lastHoveredControl = self._lastHoveredControl
	if lastHoveredControl and lastHoveredControl.OnMouseLeave then
		lastHoveredControl:OnMouseLeave()
	end
