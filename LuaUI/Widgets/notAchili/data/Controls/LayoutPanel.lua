...
function LayoutPanel:GetMaxWeight()
  ...
  for i=1, dir1 do
    for j=1, dir2 do
      ...
      if ( mweightx[ x ] or 0 ) < we then
        mweightx[ x ] = we
      end
	  
      if ( mweighty[ y ] or 0 ) < we then
        mweighty[ y ] = we
      end
...
function LayoutPanel:UpdateLayout()
  ...
  local clientW, clientH = self.clientArea[3], self.clientArea[4]

  if self.resizeItems then
    local max_ix = math.floor( clientW / self.minItemWidth  )
    local max_iy = math.floor( clientH / self.minItemHeight )

    if ( max_ix * max_iy < cn_count )
       or ( max_ix < ( self.columns or 0 ) )
       or ( max_iy < ( self.rows or 0 ) )
    then
    ...
    local childWidth  = clientW / self._columns
    local childHeight = clientH / self._rows
    local childPosx = 0
    local childPosy = 0
    ...
    for i=1, dir1 do
      for j=1, dir2 do
        ...
        if self.weightedResize then
          ...
          childWidth  = clientW * weightsx[x] / maxweightx
          childHeight = clientH * weightsy[y] / maxweighty
        else
    ...
    for i=1, cn_count do
      ...
      then
        --Spring.Echo("NotAchili-LayoutPanel: item too large (" .. child.name .. ")", totalChildWidth, clientAreaWidth)
        --//todo handle this
      end
