--//=============================================================================
--// NotAchili DebugHandler
--//=============================================================================

--// todo:
--// * use the custom errorhandler for any widget->notAchili interaction,
...
--//   nearly impossible (cuz the error mostly occurs in a ver low level notAchili object as `Font`).
--//   So any :New{} should in thoery return a wrapped table to the widgets, I already tried
--//   to wrap the whole WG.NotAchili namespace (check the `SafeWrap` function beneath), but my tests
--//   ended in crashes in notAchili, cuz such wrapped tables ended in notAchili itself
--//   (notAchili should only use links and direct ones, but never wrapped tables)


DebugHandler = {}

--//=============================================================================
--// List of all notAchili object for easy debugging
...
--// holds all created notAchili objects (uses a weaktable to do so)
...
--//=============================================================================
--// NotAchili ErrorHandler 
--//
--// NotAchili is a framework, so many widgets move parts of their code into its
--// thread. If now a widget error happens, it could be shown as notAchili errors and
--// the widgethandler would unload notAchili instead of the causing widget.
--//
--// Now this errorhandler removes the widget itself (and its owned notAchili objects)
--// w/o crashing the whole framework.

DebugHandler.maxStackLength = 7
DebugHandler.maxNotAchiliErrors = 5
local numNotAchiliErrors = 0
local lastError = 0

local function NotAchiliErrorHandler(msg,...)
  local control 
  ...
  if (control) then
    ...
    if (w) then
      Spring.Echo(("NotAchili-Error in `%s`:%s : %s"):format(w.whInfo.name, control.name, msg))
      Spring.Echo(DebugHandler.Stacktrace())

      --// this also unloads all owned notAchili objects
      Spring.Echo("Removed widget: " .. w.whInfo.name)
      widgetHandler:RemoveWidget(w)
    else
      Spring.Echo(("NotAchili-Error in `%s` (couldn't detect the owner widget): %s"):format(control.name, msg))
      Spring.Echo(DebugHandler.Stacktrace())
      control:Dispose()
    end
  else
    --// error in NotAchili itself
    Spring.Echo(("NotAchili-Error: %s"):format(msg))
    Spring.Echo(DebugHandler.Stacktrace())

    if (os.clock() - lastError < 5) then
      numNotAchiliErrors = numNotAchiliErrors + 1
    else
      numNotAchiliErrors = 1
    end
    lastError = os.clock()

    --// unload NotAchili to avoid error spam
    if (numNotAchiliErrors>=DebugHandler.maxNotAchiliErrors) then
      Spring.Echo("Removed widget: " .. widget.whInfo.name)
      widgetHandler:RemoveWidget(widget)
    end
  end
end
DebugHandler.NotAchiliErrorHandler = NotAchiliErrorHandler
...
--// we do this so we can identify it in Stacktrace() (cd = notAchili debug)
...
local function xpcall_va(func, ...)
  local n = select("#", ...)
  if (n == 0) then
    return GetResults(cdxpcall(func,NotAchiliErrorHandler))
  else
    sharedEnv._f = func
    setArgsFuncs[n](...)
    return GetResults(cdxpcall(callFuncs[n],NotAchiliErrorHandler))
  end
end


--//=============================================================================
--// Define NotAchili.SafeCall

DebugHandler.SafeCall = xpcall_va
SafeCall = xpcall_va --// external code should access it via NotAchili.SafeCall()
...
--//=============================================================================
--// NotAchili.DebugHandler.SafeWrap
--// usage:
--//   used for WG.NotAchili & screen0 to make any calling to them safe
--// how:
--//   it wraps a table, so any calling to a function in it (or ones of its subtables)
--//   uses automatically the NotAchili internal error handler
--// note:
...
--//=============================================================================
--// When a Widget gets removed destroy all its owned notAchili object, too
...
--//  so we can remove all owned notAchili object when the widget crashes/unloads
--//  (also useful for a lot of other debugging purposes)

function DebugHandler.GetWidgetOrigin(_i)
  local notAchiliwidget = widget
  ...
  repeat
    ...
    if (info.func) then
      ...
      if (_G.widget)and(_G.widget~=notAchiliwidget)and(_G.widget.GetInfo) then
    ...
    repeat
      ...
      if (name == "widget")and(value.GetInfo) then
        ...
        --// this code gets triggered when a notAchili control creates another one
...
--//=============================================================================
--// gets called by NotAchili.Object:New{}
