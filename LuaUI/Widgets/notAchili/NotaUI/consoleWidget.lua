...
function AddConsoleLine( line )
	-- hotfix to hide errors
	if (((string.find(string.lower(line), "warning") ~= nil) or (string.find(line, "GetTexture") ~= nil) or (string.find(string.lower(line), "error") ~= nil)) and (string.find(line, "Sync error") == nil)) then
		return -- skip such line
	end
			...
			if define.isPlayer then				
				...
				if player then
					...
					if player.spectator then
						result.image		= "LuaUI/Widgets/notAchili/notaUI/images/console/spectator.png"
