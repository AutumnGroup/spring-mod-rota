...
function widget:GetInfo()
  return {
    ...
    desc      = "Controls metal makers - Disable other MM widgets before use (v2.62)",
...
mmw_update         = 1.5   -- Seconds per update, do not set this too low
mmw_etargetPercent = 0.8   -- Percent of energy bar to aim for
  ...
  for _,makerStats in ipairs(metalMakers) do
    ...
    if (makerStats.state ~= makerState) then
      ...
      if (makerState == 1) then
        GiveOrderToUnit(makerStats.unitID, CMD.ONOFF, { 1 }, { })
      else
        GiveOrderToUnit(makerStats.unitID, CMD.ONOFF, { 0 }, { })
      end
    end
  end
end
...
function widget:Update(deltaTime)
  ...
  if (math.max(math.abs(oldInc-eInc), math.abs(oldUse-eUse)) > (eMax * 0.05)) then
    if (timeCounter < minUpdate) then 
  ...
  if ((math.abs(ePercent - mmw_etargetPercent) <= 0.05) and (eStable == 0)) then
    eStable = 1
    timeCounter = mmw_update
  elseif (eStable == 1) then
    if (math.abs(ePercent - mmw_etargetPercent) > 0.125) then
      eStable = 0
    end
  end
  ...
  elseif (timeCounter >= mmw_update) then
    ...
    local allies = GetTeamList(myAllyID)
    if (next(allies)) then
      if (eMaxRec == 0) then
        local allyShare = 0
        for _,allyID in ipairs(allies) do
          if (allyID ~= myTeamID) then
            local _, _, _, _, _, _, eSent = GetTeamResources(allyID, "energy")
            allyShare = allyShare + eSent
          end
        end
        if (eCur > (eMax * mmw_etargetPercent)) then
          eMaxRec = allyShare
        end
      else
        if (eMaxRec < eMax) then
          eMaxRec = eMaxRec * 1.1
        end
        if (ePercent < mmw_etargetPercent) then
          eMaxRec = eRec
        end
      end
    end
