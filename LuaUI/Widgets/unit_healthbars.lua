...
-- 24. 03. 2016
-- PepeAmpere
-- fuel bar info getting from custom res init
--------------------------------------------------------------------------------
...
attach.File("LuaRules/modules/resources/config/api/messageReceiverHealthbarsWidget.lua") -- functions reacting on received messages

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------

-- fuel structure update
fuelMaxStorage = {} -- table, key: unitDefID, value: storage
fuelStorage = {} -- table, key: unitID, value: storage
...
local barColors = {
  ...
  fuel	   = { 1.0,0.4,0.0,barAlpha },
  fuelEmpty= { 0.6,0.0,0.0,barAlpha },
  bossAlly	= { 0.0,0.7,0.0,bossBarAlpha },
  bossEnemy	= { 0.7,0.0,0.0,bossBarAlpha },
}
...
function widget:Initialize()
  ...
  if (gl.CreateShader) then
    ...
    if (barShader) then
	  ...
	  for name,width in pairs(boss) do
		  barBossDList[name] = gl.CreateList(function()
			gl.BeginEnd(GL.QUADS,function()
			  gl.Vertex(-width,0,        0,0);
			  gl.Vertex(-width,0,        width*2,0);
			  gl.Vertex(-width,bossBarHeight,width*2,0);
			  gl.Vertex(-width,bossBarHeight,0,0);

			  gl.Color(bkBottom);
			  gl.Vertex(width,0,        0,         1);
			  gl.Vertex(width,0,        width*2,1);
			  gl.Color(bkTop);
			  gl.Vertex(width,bossBarHeight,width*2,1);
			  gl.Vertex(width,bossBarHeight,0,         1);
			end)
		  end)
	  end
...
function widget:Shutdown()
  ...
  if (barDList) then
	...
	for name,width in pairs(boss) do
		gl.DeleteList(barBossDList[name])
	end
...
do
  ...
  function DrawUnitBar(offsetY,percent,color)
	...
	local bar_Height
	local bar_Width
	progress_pos= -barWidth+barWidth*2*percent-1
	bar_Height  = barHeight+offsetY
	bar_Width	= barWidth	
	
	if percent<1 then glBeginEnd(GL_QUADS,DrawGradient,progress_pos, bar_Height, bar_Width, offsetY, bkTop,bkBottom) end
	glBeginEnd(GL_QUADS,DrawGradient,-bar_Width, bar_Height, progress_pos, offsetY,brightClr,color)
	
  end
  
  function DrawBossUnitBar(offsetY,percent,color,bossName,bossSize)
    if (barShader) then
      glMultiTexCoord(1,color)
      glMultiTexCoord(2,percent,offsetY)
	  glCallList(barBossDList[bossName])
      return;
    end
	...
	progress_pos= -bossSize+bossSize*2*percent-1
	bar_Height  = bossBarHeight+offsetY
	bar_Width	= bossSize
...
do
  ...
  function AddBar(title,progress,color_index,text,color,boss,bossName)
    barsN = barsN + 1
  ...
  function DrawBars(fullText)
    ...
    for i=1,barsN do
	  ...
	  if (barInfo.boss) then 
		yoffset = 15
		DrawBossUnitBar(yoffset,barInfo.progress,barInfo.color,barInfo.bossName,boss[barInfo.bossName])
	  else
		DrawUnitBar(yoffset,barInfo.progress,barInfo.color)
	  end
      if (fullText) then
        if (barShader) then glMyText(1) end
		if(not barInfo.boss) then
			glColor(1,1,1,barAlpha)
			glText(barInfo.text,barStart,yoffset,4,"r")
		end
        if (drawBarTitles and not barInfo.boss) then
...
do
  ...
  -- local GetUnitFuel			 = Spring.GetUnitFuel -- was only in Spring 100.0 and older
  local function GetUnitFuel(unitID)
	local fuelStorage = GetUnitRulesParam(unitID, "hydrocarbonsStorage")
	if (fuelStorage ~= nil) then
		return fuelStorage
	else 
		return 0 
	end 
  ...
  local function GetUnitMaxFuel(unitDefID)
	if (fuelMaxStorage[unitDefID] ~= nil) then
		return fuelMaxStorage[unitDefID] -- check messageReceiverHealthbarsWidget.lua updating this
	else 
		return 0 
	end 
  end
  ...
  function DrawUnitInfos(unitID,unitDefID, ud)
    if not customInfo[unitDefID] then
	  ...
	  local maxFuel = GetUnitMaxFuel(unitDefID)
	  if maxFuel > 0.1 then
		customInfo[unitDefID].maxFuel = maxFuel
	  end
	  if boss[ud.name] then
		customInfo[unitDefID].boss = true
		customInfo[unitDefID].bossName = ud.name
	  end
    end
    ...
    --// BARS //-----------------------------------------------------------------------------
      ...
      --// HEALTH
      if (health) and ((drawFullHealthBars)or(hp<1)or(ci.boss)) and ((build==1)or(build-hp>=0.01)) then
        ...
        if (drawFullHealthBars)or(hp100<100)or(ci.boss) and not (hp<0) then
		  if (not ci.boss) then 
			AddBar("Health",hp,nil,(fullText and hp100..'%') or '',bfcolormap[hp100])
		  else
			local isAlly = Spring.IsUnitAllied(unitID)
			if (isAlly) then
				AddBar("Health",hp,nil,(fullText and hp100..'%') or '',bfcolormap[hp100],true,ci.bossName)
			else
				AddBar("Health",hp,"bossEnemy",(fullText and hp100..'%') or '',nil,true,ci.bossName)
			end
		  end
        end
      end
      ...
      if (emp>0.01)and(hp>0.01)and(emp<1e8) then
		...
		local seconds = floor((paralyzeDamage-maxHealth)/(maxHealth*empDecline))
	  ...
	  if ci.maxFuel then
		...
		if fuel then
			if fuel > 0 then
				fuel = fuel / ci.maxFuel
				AddBar( "Fuel", fuel, "fuel", fullText and ( floor( fuel * 100 )..'%' ) or '' )
			else
				AddBar( "Fuel", 1, "fuelEmpty", 'Empty' )
			end
		end
	  end
...
do
  ...
  function DrawFeatureInfos(featureID,featureDefID,fullText,fx,fy,fz)
	...
	if (featureIsWreck[featureID] == nil) then
		if (FeatureDefs[featureDefID or -1].tooltip == "Wreckage") then
			featureIsWreck[featureID] = true
		else
			featureIsWreck[featureID] = false
		end
	end	
	if (maxHealth and featureIsWreck[featureID]) then maxHealth = maxHealth * 1 end --!! just hotfix, how ugly PEPE!
