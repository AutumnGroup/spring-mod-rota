...
plan = {
	...
	["pathCleaner"] = function(groupID,teamNumber)
		...
		if (thisGroup.taskStatus == "startPosition") then
			local areasAlert
			if (missionKnowledge ~= nil) then areasAlert = missionKnowledge.areasAlert end
			
			-- alerts + conditions
			local someAlert		= condition.checkAlerts(areasAlert, thisTask.alertListIndexes, thisTask.alertLimit)
			...
			-- if not, continue in task
				...
				if (distanceSQ <= thisTask.limitDistance) then
					...
					if ((spotID + 1) > #choosenPath) then
						...
						local moveX,moveZ	= map[returnPoint][1],map[returnPoint][2]
						local moveY			= spGetGroundHeight(moveX,moveZ)
						commands.Move.Group(groupID,moveX,moveY,moveZ,pathType,thisGroup.formation,waiting)
					end
...
spiritDef = {
	    ...
	    if (mode == "prepare") then
		    ...
		    if (thisGroup.target.hasAny) then
				...
				if (thisTarget ~= nil) then
					...
					if (targetX ~= nil) then
						commands.Move.Group(groupID,targetX,targetY,targetZ,0,thisGroup.formation,waiting,targetX,targetZ) -- only for tests
					end
				end
			end
		end  
    ...
    ["flanker"] = function(groupID,teamNumber,mode)
		...
		if (mode == "prepare") then
			...
			if (thisGroup.target.hasAny) then
				...
				commands.Move.Group(groupID,attackPosX,0,attackPosZ,0,thisGroup.formation,waiting) -- only for tests
			end
		end
	end,
    ["lightDefender"] = function(groupID,teamNumber,mode)
        ...
        if (mode == "prepare") then
		    ...
		    if (thisGroup.target.hasAny) then
				...
				local distanceSQ = GetDistance2DSQ(thisGroup.posX,thisGroup.posZ,targetX,targetZ) 
				if (distanceSQ >= 200*200) then
		            commands.Move.Group(groupID,targetX,0,targetZ,0,thisGroup.formation,waiting,targetX,targetZ) -- only for tests
				else
				    thisGroup.target.hasAny = false
	...
	["builderDefender"] = function(groupID,teamNumber,mode)
		...
		if (not(itsLeader.sleeping)) then
			...
			commands.Move.Group(groupID,attackPosX,attackPosY,attackPosZ,0,thisGroup.formation,waiting) -- only for tests
		end
	end,
	    ...
	    if (mode == "prepare") then
		    ...
		    if (thisGroup.target.hasAny) then
				...
				if (distanceSQ < 150*150) then
		            commands.Move.Group(groupID,attackPosX,attackPosY,attackPosZ,0,thisGroup.formation,waiting,targetX,targetZ) -- only for tests
				else
				    commands.Move.Group(groupID,attackPosX,attackPosY,attackPosZ,0,thisGroup.formation,waiting)
				end
			end
		end  
	end,
    ...
    ["heavySupportLine"] = function(groupID,teamNumber,mode)
		...
		if (mode == "prepare") then
			...
			if (itsLeader.target.hasAny) then			    
				...
				local attackPosY = itsLeader.target.attackPosY
				local attackPosZ = itsLeader.target.attackPosZ
				commands.Move.Group(groupID,attackPosX - 50,attackPosY,attackPosZ,0,thisGroup.formation,waiting,targetX,targetZ) -- only for tests
			end
		end
	end,
    ...
    ["mainTank"] = function(groupID,teamNumber,mode)
	    ...
	    if (mode == "prepare") then
		    ...
		    if (thisGroup.target.hasAny) then
			    local targetX    = thisGroup.target.posX
				local targetZ    = thisGroup.target.posZ
				commands.Move.Group(groupID,targetX,0,targetZ,0,thisGroup.formation,waiting) -- only for tests
			end
		end 
	end,
		...
		if (mode == "prepare") then
			...
			if (thisGroup.membersListAlive[1] and itsLeader.membersListAlive[1]) then
				...
				if ((distanceSQ < askedDistance*askedDistance) or (enemy ~= nil)) then
					...
					commands.Move.Group(groupID,itsLeader.posX,0,itsLeader.posZ,0,thisGroup.formation,waiting, nil, nil, CMD_FIGHT) -- only for tests
				end
			end
			-- local thisTeam   = teamInfo[teamNumber]
			-- local thisGroup  = groupInfo[groupID]
    ...
    ["antigroundAir"] = function(groupID,teamNumber,mode)
		...
		if (mode == "prepare") then
			...
			if (thisGroup.target.hasAny and (thisGroup.operationalStatus ~= "IOA")) then
				...
				local targetX    = thisGroup.target.posX
				local targetZ    = thisGroup.target.posZ
			    commands.Move.Group(groupID,targetX,0,targetZ,0,thisGroup.formation,waiting)
			end
		end
	end,
