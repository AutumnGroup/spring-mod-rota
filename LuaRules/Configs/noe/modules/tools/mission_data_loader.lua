...
local function UpdateFormations()
    if (newFormationBySpirit == nil) then
	    return false
	else
...
function UpdateTables(missionPath)
	...
	attach.File(currentPath, "NONE", "mission_data_loader")
	ok = UpdateTerraformer()
	if (ok) then Spring.Echo(currentPath .. " successfully loaded") else Spring.Echo("INFO: " .. currentPath .. " NOT loaded") end
