	...
	for i=1,#teamList do
		...
		if ((isAI) and (aiInfo == "Mission AI") and (sideSettings[teamIDtoName[tostring(id)]].startMetal ~= nil)) then
			...
			if (heroClass and oteRule) then
				...
				m  = math.floor((missionInfo.playersMetal or 1000)/missionPlayersCount)
				ms = math.floor((missionInfo.playersMetal or 1000)/missionPlayersCount)
				e  = math.floor((missionInfo.playersEnergy or 1000)/missionPlayersCount) 
				es = math.floor((missionInfo.playersEnergy or 1000)/missionPlayersCount)
			end
		end
