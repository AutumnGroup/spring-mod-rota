...
function OneTypeSetSpawner(unitType, posX, posZ, facing, team, specialName, featMark, initHealthMult, formation, scale, count)
    if (team ~= nil) then
		...
		if (count <= thisFormDef.limit) then
			for i=1,count do			
				SingleSpawner(unitType, posX + thisFormation[i][1]*scale, posZ + thisFormation[i][2]*scale, facing, team, specialName, featMark, initHealthMult)
			end
			return true
		else
...
function ListSpawner(list, posX, posZ, facing, team, specialName, featMark, initHealthMult)
    if (team ~= nil) then
		for i=1,#list do
		    local thisMember = list[i]
			
			-- optional: initHealthMult, can be used as type property and also for single unit
			if (thisMember.initHealthMult ~= nil) then initHealthMult = initHealthMult * thisMember.initHealthMult end
			
			if     (thisMember.class == "single") then
			    SingleSpawner(thisMember.unit, posX + thisMember.relX, posZ + thisMember.relZ, facing, team, specialName, featMark, initHealthMult)
			elseif (thisMember.class == "buildingsSet") then
			    local thisSpawnDef = spawnDef[thisMember.name]
				if (thisSpawnDef ~= nil) then
					ListSpawner(thisSpawnDef.list, posX + thisMember.relX, posZ + thisMember.relZ, facing, team, specialName, featMark, initHealthMult)
				else
				    Spring.Echo("SpawnDefinition not found (" .. thisMember.name .. ")")
				end
			elseif (thisMember.class == "oneTypeSet") then
				OneTypeSetSpawner(thisMember.unit, thisMember.posX, thisMember.posZ, thisMember.facing, teamNames[thisMember.teamName], specialName, featMark, initHealthMult, hisMember.formation, thisMember.scale, thisMember.count)
		    end
		end
		return true
	else
...
function SingleSpawner(unit, posX, posZ, facing, team, specialName, featMark, initHealthMult)
	if (team ~= nil) then
	    local posY  = spGetGroundHeight(posX, posZ)
		local newID
		if (featMark == "feature") then
			newID = spCreateFeature(unit, posX, posY, posZ, facing, team)
			if (initHealthMult ~= 1.0) then
				local _, maxHalth = spGetFeatureHealth(newID)
				spSetFeatureHealth(newID, initHealthMult * maxHalth)
			end
		else -- if nill or "unit"
			newID = spCreateUnit(unit, posX, posY, posZ, facing, team)
			if (initHealthMult ~= 1.0) then
				local _, maxHalth = spGetUnitHealth(newID)
				spSetUnitHealth(newID, initHealthMult * maxHalth)
			end
		end
...
function Spawner(gameTime)
	...
	if (isSpawnCorrect) then
		for i=1,#spawnThis do
			...
			-- optional: initHealthMult, can be used as type property and also for single unit
			local initHealthMult  = 1.0
			if (thisMember.initHealthMult ~= nil) then initHealthMult = thisMember.initHealthMult end
			if (thisSpawnDef.initHealthMult ~= nil) then initHealthMult = thisSpawnDef.initHealthMult end
			
			--
			if (thisMember.gameTime == gameTime) then
				if (thisSpawnDef ~= nil) then
					if     (thisSpawnDef.class == "single") then
						SingleSpawner(thisSpawnDef.unit, thisMember.posX, thisMember.posZ, thisMember.facing, teamNames[thisMember.teamName], specialName, "unit", initHealthMult)
					elseif  (thisSpawnDef.class == "feature") then
						SingleSpawner(thisSpawnDef.unit, thisMember.posX, thisMember.posZ, thisMember.facing, teamNames[thisMember.teamName], specialName, "feature", initHealthMult)
					elseif (thisSpawnDef.class == "buildingsSet") then
						ListSpawner(thisSpawnDef.list, thisMember.posX, thisMember.posZ, thisMember.facing, teamNames[thisMember.teamName], specialName, "unit", initHealthMult)
					elseif (thisSpawnDef.class == "oneTypeSet") then
						OneTypeSetSpawner(thisSpawnDef.unit, thisMember.posX, thisMember.posZ, thisMember.facing, teamNames[thisMember.teamName], specialName, "unit", initHealthMult, thisSpawnDef.formation, thisSpawnDef.scale, thisSpawnDef.count)
					end
					haveIspawn = true
				else
