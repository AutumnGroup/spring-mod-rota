...
--if (modOptions.teamcom == true) then
...
function gadget:TeamDied(teamID)
	...
	if aliveTeamCount then
		...
		--printDebug( "aliveAllyTeamCount: "  .. aliveAllyTeamCount )
	...
	for _,allyTeamID in ipairs(allyTeams) do
		...
		for _,teamID in ipairs(teamList) do
			...
			if (ignoreGaia == 0) or (teamID ~= gaiaTeamID) then
				...
				--printDebug( "teamCount: "  .. teamCount )
			end
		end
...
function gadget:UnitCreated(unitID, unitDefID, unitTeamID)
	...
	--printDebug( "teamUnitCount: "  .. teamUnitCount )
	...
	--printDebug( "allyUnitCount: "  .. allyUnitCount )
