...
if(gadgetHandler:IsSyncedCode()) then
	...
	function gadget:Initialize()
		...
		goalTime = ((Spring.GetModOptions().hilltime) or 15) * 30 * 60
	...
	function gadget:GameFrame(f)
		if(f % 15 == 1) then
			for _, u in ipairs(Spring.GetUnitsInRectangle(x1, z1, x2, z2)) do
				...
				if (UnitDefs[uD] and not UnitDefs[uD].canFly) then
					TeamControl(u)
				end
			end
		...
		if(victory and timers[control] == 0 - 30 and winner == -1) then
			winner = actualTeam
		end
		if (winner ~= -1 and f % 30 == 1) then
			gameOver(winner)
		end
	end
	...
	function TeamControl(unit)
		if(control == -1) then	
			...
			for _, u in ipairs(Spring.GetUnitsInRectangle(x1, z1, x2, z2)) do
				local uD = Spring.GetUnitDefID(u)
				if(Spring.GetUnitAllyTeam(u) ~= Spring.GetUnitAllyTeam(unit)) then
					if (UnitDefs[uD] and not UnitDefs[uD].canFly) then
						hilloccupied = true
					end
				end
			end
			if (hilloccupied == false) then
