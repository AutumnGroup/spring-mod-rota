...
local ONTooltip = "Metal extractors are upgraded automatically\nby this builder." 
local OFFTooltip = "Metal extractors are NOT upgraded automatically\nby this builder." 
...
function gadget:AllowCommand(unitID, unitDefID, teamID, cmdID, cmdParams, _) 
  ...
  elseif cmdID ~= CMD_AUTOMEX then 
    if builder and getUnitPhase(unitID, teamID) == RECLAIMING then
	  if (builder.targetMex ~= nil) and (mexes[teamID][builder.targetMex] ~= nil) then
        mexes[teamID][builder.targetMex].assignedBuilder = nil 
	  end
    end 
    return true 
